...
contains
    ...
    subroutine int_least_squares_method(x_min, x_max, n, solution, solution_ext, f, a, p, q, basic, &
        ...
        do i = 1, n
            free_members_vector(i) = trapezium(fm_lsm_func, x_min, x_max, i, j, f, a, p, q, basic, 100)
        end do

        ! инициализация матрицы коэффициентов
        do i = 1, n
            do j = 1, n
                coef_matrix(i, j) = trapezium(int_lsm_func, x_min, x_max, i, j, f, a, p, q, basic, 100)
            end do
        end do
    ...
    ! -----------------------------------------------------------------------------------------------------------------
    real(dp) function int_lsm_func(x, i, j, f, a, p, q, basic) result(res)
        ...
        res = (a(x) * basic(x, j-1, 2) + p(x) * basic(x, j-1, 1) + q(x) * basic(x, j-1, 0)) * &
                (a(x) * basic(x, i-1, 2) + p(x) * basic(x, i-1, 1) + q(x) * basic(x, i-1, 0))

    end function int_lsm_func

    real(dp) function fm_lsm_func(x, i, j, f, a, p, q, basic) result(res)
        ...
        res = f(x) * (a(x) * basic(x, i-1, 2) + p(x) * basic(x, i-1, 1) + q(x) * basic(x, i-1, 0))
